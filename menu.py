...
class EmotionApp(QWidget):
    ...
    def annotate_frame(self, frame, results):
        for result in results:
            ...
            cv2.putText(frame, f'Age: {age}', (x, y + h + 20), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (255, 255, 255), 2)
            cv2.putText(frame, f'Emotion: {emotion}', (x, y + h + 40), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (255, 255, 255), 2)
            cv2.putText(frame, f'Gender: {gender}', (x, y + h + 60), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (255, 255, 255), 2)

            # Add emoji
            if emotion in self.emotion_analyzer.EMOTION_EMOJI_MAP:
                emoji_path = self.emotion_analyzer.EMOTION_EMOJI_MAP[emotion]
                frame = self.emotion_analyzer.add_emoji_to_frame(frame, emoji_path, (x, y + h + 70))
